...
| Vercel AI Gateway | `moonshotai/kimi-k3`, `openai/gpt-4.1-nano` | `alibaba/qwen3.7-flash` | Measured against the live gateway: `GET /v1/models` lists 372 ids, and a chat plus a JSON-mode call complete through `app/llm.py`. Catalog ids are `owner/model`, so the Router ids are **not** listed. The account's tier decides what a key may call — a free-tier key is refused `moonshotai/kimi-k3` with `403 Free tier users do not have access to this model`, which is entitlement rather than incompatibility; check `GET /models` (or **Test connection**) before a run |
