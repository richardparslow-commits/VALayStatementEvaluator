...
| Want Sonar models (`sonar-pro`, `sonar`) | As of 2026-09-23 they are **not served** on the Agent API (`/v1/responses`, this app's route) — live-probed; and the classic `/chat/completions` answers 403 "Sonar is now the Agent API". There is no reachable Sonar today | Re-check with `.venv/bin/python scripts/sonar_probe.py`: exit `0` = accepted → set `LLM_MODEL_MAIN=sonar-pro` / `LLM_MODEL_FAST=sonar` in `.env` (one-line switch, no code change); exit `1` = not yet; exit `2` = inconclusive (rate limit / auth / network — read the note) |
