...
|---|---|---|
| `run_app.py` | Streamlit launcher — starts the health sidecar (`app/health.py`) before handing off to `app/main.py:main` | `VA_LSE_HEALTH_PORT=0` disables the sidecar |
...
| `app/config.py` | Env vars → `Settings`, knowledge-file loader, tunable caps (`MAX_RECORD_PAGES`, `MAX_DIGEST_FACTS`, `DIGEST_CHUNK_CHARS`, upload/DOCX limits, credit defaults, `HEALTH_PORT`) | `.env` loaded with `override=True`; every cap is `VA_LSE_*`-overridable |
