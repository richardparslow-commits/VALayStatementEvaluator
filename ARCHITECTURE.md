...
|---|---|---|
| `run_app.py` | Streamlit entry point (`streamlit run run_app.py`) | Delegates to `app/main.py:main` |
...
| `app/config.py` | Env vars → `Settings`, knowledge-file loader, tunable caps (`MAX_RECORD_PAGES`, `MAX_DIGEST_FACTS`, `DIGEST_CHUNK_CHARS`, upload/DOCX limits, credit defaults) | `.env` loaded with `override=True`; every cap is `VA_LSE_*`-overridable |
...
| `app/llm.py` | OpenAI-compat client (retries, JSON parsing, token counting, `check_model_availability`) | Every call logs `phase/request_id/duration_ms/tokens`; never logs prompt/response bodies |
