...
class TestDigestCapAccounting(unittest.TestCase):
    def _facts(self, count: int) -> list[MedicalFact]:
        return [
            MedicalFact("2020-01-01", "symptom", f"distinct fact {index}", "a.pdf p.1")
            for index in range(count)
        ]

    def test_facts_dropped_by_the_cap_are_counted_on_the_digest(self) -> None:
        facts = self._facts(3)
        ...
        self.assertEqual(digest.facts_dropped_by_cap, 1)
